...
# Ensure we find the DB relative to this script
script_dir = os.path.dirname(os.path.abspath(__file__))
db_path = os.path.join(script_dir, '..', 'instance', 'dev.db')

if not os.path.exists(db_path):
    # try direct in script dir
...
try:
    ...
    c.execute('ALTER TABLE resume_data ADD COLUMN strengths_json TEXT')
    c.execute('ALTER TABLE resume_data ADD COLUMN weaknesses_json TEXT')
except Exception as e:
    print(f"ResumeData update issue (might already exist): {e}")
